...
Модуль `valid_mem` представляет из себя регистровую память, хранящую 16 бит (4-битный адресс), ровно как и уникальных значений ID.

Применительно к данному дизайну, каждый адрес соответсвует одному биту данных, который сигнализирует о валидности/невалидности данных в основном модуле памяти, это достигается за счёт того, что при записи в основную память по такому же адресу в `valid_mem` записывается единица, что сигнализирует о том, что по данному адрессу были записаны корректные данные.
...
<!-- TODO Оценка производительности -->
